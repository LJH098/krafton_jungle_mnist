...
class BatchNorm:
    ...
    def backward(self, dout):
        ...
        dvar = np.sum(dx_norm * self.xc * -0.5 * (self.std ** -3), axis=0)
        dmu = np.sum(dx_norm * -1 / self.std, axis=0) + dvar * np.mean(-2 * self.xc, axis=0)
        dx = dx_norm / self.std + dvar * 2 * self.xc / self.batch_size + dmu / self.batch_size
